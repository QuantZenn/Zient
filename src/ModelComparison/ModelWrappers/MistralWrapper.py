...
class MistralWrapper:
    ...
    def _predict_single(self, text: str) -> dict:
        ...
        followup_prompt = (
            ...
            f"Sentiment: {result['predicted_label']}"
        )
        ...
        result["confidence"] = self._extract_qualitative_confidence(followup_decoded)
        return result
    ...
    def predict_batch(self, input_data: Union[List[str], pd.DataFrame, str, Dataset]) -> pd.DataFrame:
        ...
        predictions = [self._predict_single(text) for text in texts]

        df["predicted_label"] = [p["predicted_label"] for p in predictions]
        df["confidence"] = [p["confidence"] for p in predictions]

        # If original had 'label' (ground truth), retain it
        if "label" in df.columns:
            result_df = df[["text", "label", "predicted_label", "confidence"]]
        else:
            result_df = df[["text", "predicted_label", "confidence"]]
